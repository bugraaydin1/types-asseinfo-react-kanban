...
function ReactKanban() {
	...
	const ColumnHeader: ColumnHeaderComponent = ({ column }) => {
		return (
			<div>
				{column.title}
				<button onClick={() => removeColumn(board, column)}>
					Remove Column
				</button>
				<button
					onClick={() =>
	...
	return (
		<Board
			...
			renderCard={(card, cardBag) => (
				<CardComponent card={card} cardBag={cardBag} />
			)}
			renderColumnHeader={(column, columnBag) => (
				<ColumnHeader column={column} columnBag={columnBag} />
			)}
